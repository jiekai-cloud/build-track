...
async function fix() {
  // Let's also check inventory, purchaseOrders
  const collections = ["inventoryItems", "inventoryLocations", "purchaseOrders"];
  for (const collection of collections) {
    const { data, error } = await supabase.from("app_data").select("item_id, data").eq("collection", collection);
    if (error) { console.error(error); continue; }
    
    // Some older models might not even have departmentId field, or have null
