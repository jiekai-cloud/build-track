...
已為您準備好最新的更新部署檔案（版本 V4.12 - 印章位置最終版）：
- **檔案名稱**：`build-track-update-v4.12-stamp-position-final.zip`
- **修正功能**：
  1. **印章精確定位**：根據您的指示，將印章移動至銀行帳號下方的空白處，且向左些微偏移，使其位置更加自然且不遮擋文字。
  2. **印章完全整合**：內建圖片Base64編碼，確保顯示無虞。
  3. **報價單專用章**：支援在預覽與 PDF 列印中顯示。
  4. **聯絡人功能**（V4.6）：自動帶入並顯示聯絡人。
  5. **優化功能**：包含先前所有的列表、字體與表格優化。
