...
已為您準備好最新的更新部署檔案（版本 V4.3 - 報價單表格優化）：
- **檔案名稱**：`build-track-update-v4.3-pdf-style.zip`
- **新增功能**：
  1. **報價單表格排版**：將表格標題列改為簡約的灰底風格，並且讓分隔線顏色變淡，解決先前「縱向黑線過於突兀」的問題。
  2. **版面優化**（V4.2）：公司名稱縮小並靠左對齊。
  3. **可編輯備註**（V4.1）：標準庫備註可自由編輯。
  4. **工項複製**（V4.0）：支援快速複製工項。
