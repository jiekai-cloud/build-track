...
已為您準備好最新的更新部署檔案（版本 V4.17 - 精細還原版）：
- **檔案名稱**：`build-track-update-v4.17-granular-restore.zip`
- **重要更新**：
  1. **全面精細還原功能**：現在「匯入現有資料」時，所有類別（不只是專案，還包含報價單、客戶、廠商、庫存...等）都出現了「挑選」按鈕。
     - 您可以點擊「挑選」預覽備份檔內的詳細清單。
     - 勾選特定幾筆資料進行還原，而不需要整批覆蓋。
  2. **修復與強化**（V4.16）：報價單還原功能現已完整支援。
- **新增功能**：
...
2. 選擇 `build-track-update-v4.17-granular-restore.zip`
3. 等待上傳完成（約 1-2 分鐘）
