...
已為您準備好最新的更新部署檔案（版本 V4.19 - 完整備份修復版）：
- **檔案名稱**：`build-track-update-v4.19-full-backup.zip`
- **緊急修復**：
  1. **手動備份完整性修復**：發現之前的「匯出備份 (JSON)」功能漏掉了報價單、庫存、廠商等重要資料。V4.19 已修正此問題，現在匯出的備份檔將包含**所有**系統資料。
  2. **強烈建議**：請立即下載此版本並執行一次新的手動備份，以確保您擁有完整的資料存檔。
- **體驗優化**：
...
2. 選擇 `build-track-update-v4.19-full-backup.zip`
3. 等待上傳完成（約 1-2 分鐘）
