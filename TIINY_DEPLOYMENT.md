...
已為您準備好最新的更新部署檔案（版本 V4.7 - 報價單專用章）：
- **檔案名稱**：`build-track-update-v4.7-quotation-stamp.zip`
- **新增功能**：
  1. **報價單專用章**：支援在報價單預覽與 PDF 列印中顯示「報價專用章」。
     - **重要**：請將您的印章圖檔命名為 `stamp.png` 並放入專案的 `public` 資料夾，或者直接將檔名為 `stamp.png` 的圖片放入解壓縮後的部署資料夾中，印章才會顯示（建議去背 PNG 檔）。
  2. **報價單聯絡人**（V4.6）：自動帶入專案聯絡人至報價單。
  3. **新增專案聯絡人**（V4.5）：編輯案件資訊時可新增聯絡人。
  4. **列表顯示優化**（V4.5）：列表模式顯示聯絡人。
  5. **字體調整**（V4.4）：優化報價單標題與單號字體大小。
  6. **表格優化**（V4.3）：柔和化表格分隔線與標題背景。
  1. **刪除大項**：現在可以刪除整個工程分類（大項）。
  2. **編輯大項編號**：可以直接點擊並修改「壹、貳...」等分類編號。
  3. **跨大項移動**：支援將工項拖曳移動到不同的大項分類中（也可拖曳至空分類）。
