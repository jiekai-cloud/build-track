...
已為您準備好最新的更新部署檔案（版本 V4.8 - 印章路徑修正）：
- **檔案名稱**：`build-track-update-v4.8-stamp-fix.zip`
- **修正功能**：
  1. **印章路徑修正**：強化了報價專用章的圖片載入邏輯，解決部分環境下無法讀取 `stamp.png` 的問題。
  2. **內建印章**：部署檔案中已包含 `stamp.png`。
  3. **報價單專用章**（V4.7）：支援在報價單 PDF 列印中顯示「報價專用章」。
  4. **報價單聯絡人**（V4.6）：自動帶入專案聯絡人至報價單。
  5. **新增專案聯絡人**（V4.5）：編輯案件資訊時可新增聯絡人。
  6. **列表顯示優化**（V4.5）：列表模式顯示聯絡人。
