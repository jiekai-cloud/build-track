...
已為您準備好最新的更新部署檔案（版本 V4.6 - 報價單聯絡人）：
- **檔案名稱**：`build-track-update-v4.6-quotation-contact.zip`
- **新增功能**：
  1. **報價單聯絡人**：在編輯報價單時，會自動帶入專案設定的聯絡人資訊，也可以手動修改。
  2. **新增專案聯絡人**（V4.5）：編輯案件資訊時可新增聯絡人。
  3. **列表顯示優化**（V4.5）：列表模式顯示聯絡人。
  4. **字體調整**（V4.4）：優化報價單標題與單號字體大小。
  5. **表格優化**（V4.3）：柔和化表格分隔線與標題背景。
  6. **版面優化**（V4.2）：公司名稱縮小並靠左對齊。
