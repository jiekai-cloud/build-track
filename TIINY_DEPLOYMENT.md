...
已為您準備好最新的更新部署檔案（版本 V4.5 - 專案聯絡人）：
- **檔案名稱**：`build-track-update-v4.5-project-contact.zip`
- **新增功能**：
  1. **新增專案聯絡人**：在編輯案件資訊時，可以新增「案件聯絡人」(例如：委員或總幹事)，方便記錄管委會或其他單位的對接窗口。
  2. **列表顯示優化**：在專案列表（卡片、表格、看板模式）中可以直接看到案件聯絡人資訊。
  3. **字體調整**（V4.4）：優化報價單標題與單號字體大小。
  4. **表格優化**（V4.3）：柔和化表格分隔線與標題背景。
  5. **版面優化**（V4.2）：公司名稱縮小並靠左對齊。
  6. **可編輯備註**（V4.1）：標準庫備註可自由編輯。
