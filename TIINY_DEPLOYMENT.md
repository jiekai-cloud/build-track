...
已為您準備好最新的更新部署檔案（版本 V4.9 - 印章完全整合版）：
- **檔案名稱**：`build-track-update-v4.9-stamp-embedded.zip`
- **修正功能**：
  1. **印章完全整合**：已將報價專用章直接嵌入程式碼中，無需依賴圖片路徑，解決所有圖片載入失敗的問題。
  2. **內含圖檔**：雖然已嵌入，但部署檔中仍保留 `stamp.png` 作為備份。
