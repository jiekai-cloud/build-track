...
已為您準備好最新的更新部署檔案（版本 V4.18 - 全選還原版）：
- **檔案名稱**：`build-track-update-v4.18-bulk-select.zip`
- **體驗優化**：
  1. **一鍵全選功能**：在「匯入現有資料」視窗右上角新增了「全選」與「全取消」按鈕，讓您可以快速勾選所有類別，不需要一個一個點擊。
  2. **全面精細還原功能**（V4.17）：所有類別皆支援「挑選」模式。
- **新增功能**：
...
2. 選擇 `build-track-update-v4.18-bulk-select.zip`
3. 等待上傳完成（約 1-2 分鐘）
