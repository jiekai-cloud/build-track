...
已為您準備好最新的更新部署檔案（版本 V4.15 - 資料安全性修復版）：
- **檔案名稱**：`build-track-update-v4.15-data-persistence-fix.zip`
- **重要修復**：
  1. **資料保存強化**：修復報價單偶爾會回朔到舊版本的問題。現在系統會同時寫入 IndexedDB 與 LocalStorage 進行雙重備份。
  2. **自動存檔一致性**：將報價單存檔邏輯整合至主系統自動存檔流程，避免時間差導致的資料遺失。
- **新增功能**：
...
2. 選擇 `build-track-update-v4.15-data-persistence-fix.zip`
3. 等待上傳完成（約 1-2 分鐘）
